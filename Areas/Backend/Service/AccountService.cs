...
namespace BASE.Areas.Backend.Service
{
    public class AccountService : ServiceBase
    {
        ...
        /// </summary>
        /// <returns></returns>
        public IQueryable<AccountExtend>? GetAccountExtendList(ref String ErrMsg, VM_AccountQueryParam? vmParam, Expression<Func<AccountExtend, bool>>? filter = null)
        {
            try
            {
                IQueryable<AccountExtend> dataList = (from account in _context.TbUserInfo
                                                      ...
                                                      select new AccountExtend
                                                      {
                                                          account = account,
                                                          accountGroup = account.TbUserInGroup.FirstOrDefault(),
                                                          groupInfo = gp
                                                      });
                ...
                if (vmParam != null)
                {
                    ...
                    {
                        //關鍵字搜尋：使用者名稱
                        dataList = dataList.Where(x => x.account.UserName.Contains(vmParam.Keyword));
                    }

                    if (!string.IsNullOrEmpty(vmParam.sGroup))
                    {
                        //關鍵字搜尋：群組
                        dataList = dataList.Where(x => x.account.TbUserInGroup.FirstOrDefault().GroupId == vmParam.sGroup);
                    }
                }
                return dataList;
            }
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public IQueryable<AccountExtend>? GetAccountExtendItem(ref string ErrMsg, string id)
        {
            try
            {
                IQueryable<AccountExtend>? dataList = (from account in _context.TbUserInfo
        ...
        /// </summary>
        /// <returns></returns>
        public IQueryable<OperateExtend>? GetOperateExtendList(ref String ErrMsg, VM_AccountQueryParam? vmParam, Expression<Func<OperateExtend, bool>>? filter = null)
        {
            try
            {
                ...
                if (vmParam != null)
                {
                    if (!string.IsNullOrEmpty(vmParam.sAccount))
                    {
                        //關鍵字搜尋：操作帳號
                        dataList = dataList.Where(x => x.User.UserId.Contains(vmParam.sAccount));
                    }
